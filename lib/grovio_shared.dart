...
Widget grovioNetworkImage({
  ...
  if (kIsWeb) {
    return Image.network(
      ...
      loadingBuilder: (context, child, loadingProgress) {
        ...
        return placeholder ?? Container(color: Colors.grey.shade50);
      },
  ...
  return CachedNetworkImage(
    ...
    placeholder: (context, url) =>
        placeholder ?? Container(color: Colors.grey.shade50),
...
class GrovioFirestore {
  ...
  // Store Methods
  static Stream<List<GroceryItem>> getProducts({String? category, int limit = 50}) {
    Query query = _db.collection('products');
    if (category != null && category != "cat_all") {
      query = query.where('category', isEqualTo: category);
    }
    // Limit results for low-RAM devices and faster loading
    return query.limit(limit).snapshots().map(
      (snap) =>
          snap.docs.map((doc) => GroceryItem.fromMap(doc.data() as Map<String, dynamic>)).toList(),
    );
  }

  static Future<List<GroceryItem>> getProductsFuture({String? category, DocumentSnapshot? startAfter, int limit = 20}) async {
    Query query = _db.collection('products').orderBy('id');
    if (category != null && category != "cat_all") {
      query = query.where('category', isEqualTo: category);
    }
    if (startAfter != null) {
    ...
    return snap.docs.map((doc) => GroceryItem.fromMap(doc.data() as Map<String, dynamic>)).toList();
  }
  ...
  static Stream<List<Map<String, dynamic>>> getFeedbacks({
  ...
  static Future<void> replyToFeedback(String feedbackId, String reply) {
  ...
  static Future<GroceryItem?> getProductByBarcode(String barcode) async {
    ...
    if (snap.docs.isNotEmpty) {
      return GroceryItem.fromMap(snap.docs.first.data());
    }
    return null;
  }
  ...
  static Future<void> saveFeedback({
    ...
    return _db.collection('feedback').doc(id).set({
      'id': id,
  ...
  // Expense Methods
  static Future<void> saveExpense(GrovioExpense expense) {
    return _db.collection('expenses').doc(expense.id).set(expense.toMap());
  }

  static Stream<List<GrovioExpense>> getExpenses() {
    return _db
        .collection('expenses')
        .orderBy('date', descending: true)
        .snapshots()
        .map(
          (snap) => snap.docs
              .map((doc) => GrovioExpense.fromMap(doc.data()))
              .toList(),
        );
  }
}
...
class GrovioExpense {
  ...
  factory GrovioExpense.fromMap(Map<String, dynamic> map) => GrovioExpense(
    id: map['id'] ?? "",
...
class GrovioStrings {
  ...
  static final Map<String, Map<String, String>> _data = {
    ...
    "hi": {
      ...
      "feedback_hint": "हमें बताएं कि हम कैसे सुधार कर सकते हैं...",
      "submit": "सबमिट करें",
  ...
  static String get(String key) {
    String res = _data[lang]?[key] ?? key;
    if (res == key && key.startsWith("cat_")) {
      return key
          .replaceFirst("cat_", "")
          .replaceAll("_", " ")
          .split(' ')
